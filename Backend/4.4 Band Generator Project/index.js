...
app.post("/submit", (req, res) => {
  let randmBandName = getRandomBandName();
  const date = new Date();
  let year = date.getFullYear;
  res.render("index.ejs", { bandName: randmBandName, currentYear: year });
...
function getRandomBandName() {
  const randomAdjNumber = Math.floor(Math.random() * adj.length);
  const randomNounNumber = Math.floor(Math.random() * noun.length);

  const bandName = `${adj[randomAdjNumber]} ${noun[randomNounNumber]}`;
